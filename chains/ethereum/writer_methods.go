	...
	for i := 0; i < TxRetryLimit; i++ {
		...
		default:
			...
			if err == nil {
				...
				reqURL := fmt.Sprintf("http://localhost:3000/transactions/execute-proposal/%s", strconv.FormatUint(uint64(m.DepositNonce), 10))
				w.log.Info("EXECUTE_PROPOSAL")
				w.log.Info(reqURL)
				_, err := http.Get(reqURL)
				if err != nil {
					w.log.Error(err.Error())
